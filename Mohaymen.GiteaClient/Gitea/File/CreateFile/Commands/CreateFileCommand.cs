...
internal class CreateFileCommandHandler : IRequestHandler<CreateFileCommand, ApiResponse<CreateFileResponseDto>>
{
    ...
    public async Task<ApiResponse<CreateFileResponseDto>> Handle(CreateFileCommand request, CancellationToken cancellationToken)
    {
        _validator.ValidateAndThrow(request);
        var createFileRequest = request.ToCreateFileRequest();
        var owner = _options.Value.RepositoriesOwner;
        return await _fileRestClient.CreateFileAsync(owner, request.RepositoryName, request.FilePath, createFileRequest, cancellationToken)
